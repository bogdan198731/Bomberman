...
import {
  applyCircuitResult,
  CIRCUIT_ELIGIBLE_GAMES,
  CIRCUIT_LENGTH,
  CIRCUIT_STORAGE_KEY,
  circuitCurrentGame,
  circuitIsComplete,
  circuitPerformancePercent,
  circuitStagePoints,
  circuitTotalPoints,
  circuitWins,
  createArcadeCircuit,
...
test('circuit points balance outcomes and cap raw game scores', () => {
  assert.equal(circuitStagePoints({ outcome: 'win', score: 6_000 }, 'blocks'), 1_000);
  assert.equal(circuitStagePoints({ outcome: 'complete', score: 1_024 }, 'twenty48'), 750);
...
test('a new circuit contains three different arcade games', () => {
  const circuit = createArcadeCircuit('solo', () => 0.25, 123);
...
test('daily lineups are shared by mode and friends never receive solo puzzles', () => {
  const first = createArcadeCircuit('solo', undefined, 1_700_000_000_000);
  const second = createArcadeCircuit('solo', undefined, 1_700_000_000_500);
  const friends = createArcadeCircuit('friends', () => 0, 1_700_000_000_000);
  assert.deepEqual(second.lineup, first.lineup);
  assert.equal(second.rulesId, first.rulesId);
  assert.ok(friends.lineup.every(gameId => CIRCUIT_ELIGIBLE_GAMES.friends.includes(gameId)));
  assert.equal(friends.lineup.includes('twenty48'), false);
  assert.equal(friends.lineup.includes('sudoku'), false);
});

test('performance normalization makes unlike game score units comparable', () => {
  assert.equal(circuitPerformancePercent('paddle', 7), 100);
  assert.equal(circuitPerformancePercent('blocks', 6_000), 100);
  assert.equal(circuitStagePoints({ outcome: 'win', score: 7 }, 'paddle'), 1_000);
  assert.equal(circuitStagePoints({ outcome: 'win', score: 6_000 }, 'blocks'), 1_000);
});

test('only the current circuit game can advance a run', () => {
  let progress = startArcadeCircuit(createDefaultCircuitProgress(), 'solo', () => 0, 10);
...
test('finishing a circuit records its best score and completed-run count', () => {
  let progress = startArcadeCircuit(createDefaultCircuitProgress(), 'solo', () => 0.6, 1);
...
test('circuit state normalization repairs scores and stops invalid result sequences', () => {
  ...
  assert.equal(normalized.current?.results[0].points, 635);
...
test('circuit progress round-trips through browser-style storage', () => {
  ...
  const progress = startArcadeCircuit(createDefaultCircuitProgress(), 'solo', () => 0.4, 44);
