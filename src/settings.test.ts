...
test('settings normalization repairs invalid values and clamps volume', () => {
  assert.deepEqual(normalizeSettings({ soundEnabled: false, volume: 140, reducedMotion: true, highContrast: 'yes' }), {
    soundEnabled: false,
    volume: 100,
    reducedMotion: true,
    highContrast: false,
...
test('settings round-trip through browser-style storage', () => {
  ...
  saveSettings({ soundEnabled: false, volume: 35, reducedMotion: true, highContrast: true }, storage);
  assert.ok(values.has(SETTINGS_STORAGE_KEY));
  assert.deepEqual(loadSettings(storage), { soundEnabled: false, volume: 35, reducedMotion: true, highContrast: true });
});
